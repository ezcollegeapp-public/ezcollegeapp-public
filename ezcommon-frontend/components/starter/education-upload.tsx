...
export function EducationUpload() {
  ...
  useEffect(() => {
    ...
    if (!userId) return
    const base = process.env.NEXT_PUBLIC_BACKEND_URL || '/api/backend'
    fetch(`${base}/api/upload/education?user_id=${encodeURIComponent(userId)}`)
      .then((r) => r.json())
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">Upload your transcript. We will extract your school and academic info so you can review and edit.</p>
      </div>
      ...
      >
        <p className="mb-4 text-sm text-muted-foreground">Drag and drop transcripts here (PDF or image), or click to select. Multiple files supported.</p>
        <input
      ...
      <div className="flex flex-col gap-3">
        ...
        <Link href="/starter/activity/upload" className="text-center text-sm text-muted-foreground hover:underline">Skip, I will manually input</Link>
      </div>
